...
def geneticAlghoritm(y1, y2, image, pop_num=10, deltaX=20, epoch=500):
    ...
    for p in range(epoch):                    #Это наш основной цикл. Сейчас стоит 60 итераций. Т.е. мы ставим ограничение в 60 поколений.
        ...
        #         Father.A[j]=pop1.B[tt].A[j] #А папами пусть будет first 5 индивидов нашей популяции 
        for s in range(0,Line.pop_num,2):                    #Цикл. От 0 до 10 с шагом 2. Т.е. тут мы берём 5 пар родителей.
            for j in range(Line.length):                    #Как ты заметил, цикл из 30 итераций есть практически везде. Т.к. все операции мы проводим поэлементно (большая честь каждому нолику и каждой единичке).
                rand = random.randint(0, Line.pop_num)
                Mother.A[j]=pop1.B[random.randint(0, Line.pop_num-1)].A[j]      #Пусть мамами у нас будут последние 5 индивидов нашей популяции (т.к. они у нас в последствии всегда будут отранжированы (наши популяции), беря последние 5, мы тем самым берём лучших представителей и с ними работаем. Чего с посредственностей-то взять, ну.
                Father.A[j]=pop1.B[random.randint(0, Line.pop_num-1)].A[j] #А папами пусть будет first 5 индивидов нашей популяции 
            ...
            for i in range(Line.length):                    #Тут мы закидываем наших получившихся в результате скрещивания s-той пары родителей Сына1 и Сына2 во вторую половину массива "Отцы и дети".
                ParAndSons[Line.pop_num+2*s].A[i]=Son1.A[i]
                ParAndSons[Line.pop_num+2*s+1].A[i]=Son2.A[i]
                ParAndSons[Line.pop_num+2*s+2].A[i]=Son3.A[i]
                ParAndSons[Line.pop_num+2*s+3].A[i]=Son4.A[i]
        ...
        for r in range(Line.pop_num*3, 5):                
        ...
        for i in range(Line.pop_num*3):                     #Это важно. Далее мы будем ранжировать массив отцов и детей в порядке возрастания живучести (суммы (fit)).
